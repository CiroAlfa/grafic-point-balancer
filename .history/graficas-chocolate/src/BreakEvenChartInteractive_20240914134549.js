...
const BreakEvenChartInteractive = () => {
  ...
  useEffect(() => {
    const calcularPuntoEquilibrio = () => {
      const x = (costoFijo / (precioVenta - costoVariable)).toFixed(2);  // Mantiene dos decimales en el punto de equilibrio
      const y = (x * precioVenta).toFixed(2);  // Mantiene dos decimales en el ingreso
      setPuntoEquilibrio({ x: parseFloat(x), y: parseFloat(y) });  // Convertimos a número decimal
      generarDatos(x, y);
    };
